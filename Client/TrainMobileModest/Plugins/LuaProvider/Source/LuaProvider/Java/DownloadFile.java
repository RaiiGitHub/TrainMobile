...
public class DownloadFile extends Thread {
    ...
    public DownloadFile(String url, String name, long startPos, long endPos, int threadId) throws IOException {
        ...
        this.itemFile = new SaveItemFile(name, startPos);//open this section.
    }
    @Override
    public void run() {
        while (endPos > startPos && !isDownloadOver) {
            try {
                ...
                while ((length = is.read(buff)) > 0 && startPos < endPos && !isDownloadOver) {
                    //写入文件内容，返回最后写入的长度
                    int write_len = itemFile.write(buff, 0, length);
                    startPos += (long)write_len;
                    ApkDownloader.notifyDownloadProcess(write_len);
                }
                ...
                LogUtils.log("Thread "+threadId+" -MalformedURLException");
                e.printStackTrace();
            } catch (IOException e) {
                LogUtils.log("Thread "+threadId+" -IOException");
                e.printStackTrace();
            } finally {
                LogUtils.log("Thread "+threadId+" do fianlly jobs.");
            }
        }
    ...
    public static void printHeader(URLConnection conn) {
        ...
        while (true) {
            ...
            i++;
            if (header != null) {
                LogUtils.log(header + ":" + conn.getHeaderField(i));
            } else {
                break;
            }
        }
    ...
    public static void setHeader(URLConnection conn) {
        ...
        conn.setRequestProperty("Referer", "http://www.resafety.com/mobile/");
    }
